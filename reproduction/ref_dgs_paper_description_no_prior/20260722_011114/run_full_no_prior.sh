...
LABEL='PAPER-DESCRIPTION NO-GEOMETRY-PRIOR EXPERIMENT — NOT UNMODIFIED OFFICIAL CODE'
RUN_ROOT=/data1/liuly/reproduction/ref_dgs_paper_description_no_prior/20260722_011114
...
exec > >(tee -a "$RUN_ROOT/logs/full_runner.log") 2>&1
...
run_scene() {
  ...
  local train_rc render_rc train_pid render_pid reuse_train=0
  local extra_args=("$@")
  ...
  if [[ -e "$attempt" ]]; then
    if [[ -s "$final_pc/gaussians_point_cloud.ply" &&
          -s "$final_pc/ref_gaussians_point_cloud.ply" &&
          -s "$final_pc/light_mlp.pt" &&
          -s "$final_pc/dir_encoding.pt" &&
          -f "$log_dir/train_exit_code.txt" ]] &&
       [[ "$(cat "$log_dir/train_exit_code.txt")" == 0 ]] &&
       rg -q 'Training complete' "$train_log" &&
       rg -q '^runtime_diff_sha256=' "$attempt/frozen_identity.txt" &&
       rg -q '^geometry_prior_loader_disabled=1$' "$attempt/frozen_identity.txt" &&
       rg -q '^vggt_weight=0$' "$attempt/frozen_identity.txt" &&
       rg -q '^vggt_until_iter=0$' "$attempt/frozen_identity.txt"; then
      reuse_train=1
      echo "reusing_identity_matched_completed_training dataset=$dataset scene=$scene"
    else
      echo "refusing_to_overwrite_or_reuse_incomplete_attempt dataset=$dataset scene=$scene path=$attempt"
      return 1
    fi
  fi
  [[ ! -e "$RUNTIME/priors" ]] || { echo "runtime_priors_must_be_absent"; return 1; }
  if (( reuse_train == 0 )); then
  ...
  mkdir -p "$model" "$log_dir"
  {
    ...
    echo "refdgs_base_commit=$(git -C "$RUNTIME" rev-parse HEAD)"
    echo "runtime_diff_sha256=$(sha256sum "$RUN_ROOT/code_changes.diff" | cut -d' ' -f1)"
    echo "geometry_prior_loader_disabled=1"
    echo "vggt_weight=0"
    echo "vggt_until_iter=0"
    printf 'extra_args='; printf '%q ' "${extra_args[@]}"; echo
  } > "$attempt/frozen_identity.txt"
  {
    echo "#!/usr/bin/env bash"
    echo "# $LABEL"
    printf 'conda run -n refdgs env CUDA_VISIBLE_DEVICES=%q REFDGS_DISABLE_GEOMETRY_PRIOR=1 REFDGS_NO_PRIOR_LOSS_LOG=1 PYTHONPATH=%q python %q -s %q -m %q --eval --iterations %q --run_dim 64 --vggt_weight 0 --vggt_until_iter 0' \
      "$GPU_PHYSICAL" "$RUNTIME" "$RUNTIME/train.py" "$source" "$model" "$iterations"
  ...
  (
    ...
    /usr/bin/time -v conda run -n refdgs env \
      ...
      REFDGS_DISABLE_GEOMETRY_PRIOR=1 \
      REFDGS_NO_PRIOR_LOSS_LOG=1 \
      PYTHONPATH="$RUNTIME" \
      python "$RUNTIME/train.py" -s "$source" -m "$model" --eval \
      --iterations "$iterations" --run_dim 64 --vggt_weight 0 --vggt_until_iter 0 "${extra_args[@]}"
  ) > "$train_log" 2>&1 &
  train_pid=$!
  ...
  rg -q 'Training complete' "$train_log"
  ! rg -q 'FloatingPointError|CUDA out of memory|(^|[^A-Za-z])(nan|NaN|inf|Inf)([^A-Za-z]|$)' "$train_log"
  ...
  local early_records disabled_records
  early_records=$(rg -c '\[NO_GEOMETRY_PRIOR_LOSS\].*"iteration": ([1-9]|[1-9][0-9]|100),' "$train_log" || true)
  disabled_records=$(rg -c '\[NO_GEOMETRY_PRIOR_LOSS\].*"geometry_prior_loaded": false.*"vggt_until_iter": 0.*"vggt_weight": 0\.0' "$train_log" || true)
  (( early_records == 100 )) || { echo "early_loss_record_count_failed count=$early_records"; return 1; }
  (( disabled_records >= 100 )) || { echo "no_prior_record_count_failed count=$disabled_records"; return 1; }
  echo "train_complete dataset=$dataset scene=$scene iterations=$iterations early_records=$early_records"
  else
    local early_records disabled_records
    early_records=$(rg -c '\[NO_GEOMETRY_PRIOR_LOSS\].*"iteration": ([1-9]|[1-9][0-9]|100),' "$train_log" || true)
    disabled_records=$(rg -c '\[NO_GEOMETRY_PRIOR_LOSS\].*"geometry_prior_loaded": false.*"vggt_until_iter": 0.*"vggt_weight": 0\.0' "$train_log" || true)
    (( early_records == 100 )) || { echo "reused_early_loss_record_count_failed count=$early_records"; return 1; }
    (( disabled_records >= 100 )) || { echo "reused_no_prior_record_count_failed count=$disabled_records"; return 1; }
  fi
  ...
  storage_gate "$RENDER_HEADROOM_BYTES"
  {
    echo "#!/usr/bin/env bash"
    echo "# $LABEL"
    printf 'conda run -n refdgs env CUDA_VISIBLE_DEVICES=%q REFDGS_DISABLE_GEOMETRY_PRIOR=1 PYTHONPATH=%q python %q -m %q --iteration %q --dataset %q --voxel_size 0.002\n' \
  ...
  (
    cd "$RUNTIME"
    /usr/bin/time -v conda run -n refdgs env CUDA_VISIBLE_DEVICES="$GPU_PHYSICAL" \
      REFDGS_DISABLE_GEOMETRY_PRIOR=1 PYTHONPATH="$RUNTIME" \
...
echo "# Executed commands — $LABEL" >> "$RUN_ROOT/commands.sh"
