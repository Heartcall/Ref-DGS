...
import random
from scene import Scene, GaussianModel, RefGaussianModel
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint):
    ...
    for iteration in range(first_iter, opt.iterations + 1):     
        ...
        ref_render_pkg = render_ref(viewpoint_cam, ref_gaussians, pipe, bg, render_pkg)
        ...
        gt_mask = viewpoint_cam.gt_alpha_mask  # float
        ...
        if iteration < opt.vggt_until_iter:
            ...
            vggt_normal_loss = opt.vggt_weight * (
                viewpoint_cam.vggt_depth_conf * (cosine_term + 0.5 * l1_term)
            ).sum() / torch.sum(gt_mask>0.5)
            vggt_depth_loss = opt.vggt_weight * depth_prior_loss(render_pkg["surf_depth"], viewpoint_cam.vggt_depth, viewpoint_cam.vggt_depth_conf, (gt_mask>0.5))
        ...
        if os.environ.get("REFDGS_DIAGNOSTIC_LOSS_LOG") == "1":
            ...
            should_emit = iteration <= 100 or iteration % 500 == 0 or iteration in saving_iterations
            if should_emit:
        ...
        if iteration%500 == 0:
            with torch.no_grad():   
                save_image(iteration, dataset.model_path, viewpoint_cam, render_pkg, ref_render_pkg, pbr_rgb)
        ...
        with torch.no_grad():
            ...
            #training_report(tb_writer, iteration, Ll1, loss, loss_pbr, alpha_loss, dist_loss, normal_loss, vggt_normal_loss, vggt_depth_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, render_ref, pipe, background)
            ...
            if iteration < opt.refgs_densify_until_iter:
            ...
            if (iteration in checkpoint_iterations):
                ...
                torch.save((gaussians.capture(), iteration), scene.model_path + "/chkpnt" + str(iteration) + "gaussians.pth")
...
def training_report(tb_writer, iteration, Ll1, loss, loss_pbr, alpha_loss, dist_loss, normal_loss, vggt_normal_loss, vggt_depth_loss,
                    elapsed, testing_iterations, scene : Scene, renderFunc, renderrefFunc, pip, bg):
    if tb_writer:
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    render_pkg = renderFunc(viewpoint, scene.gaussians, pip, bg)
                    ref_render_pkg = renderrefFunc(viewpoint, scene.ref_gaussians, pip, bg, render_pkg)
                    image = get_final_color(render_pkg, ref_render_pkg, bg)
        
                    #image = torch.clamp(render_pkg["pbr_rgb"], 0.0, 1.0)
...
if __name__ == "__main__":
    ...
    #                     [5_000, 7_000, 10_000, 15_000, 20_000, 25_000, 30_000]
    #                    )
    # parser.add_argument("--save_iterations", nargs="+", type=int, default=
    #                     [5_000, 7_000, 10_000, 15_000, 20_000, 25_000, 30_000]
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[15_000, 20_000, 25_000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[15_000, 20_000, 25_000])
